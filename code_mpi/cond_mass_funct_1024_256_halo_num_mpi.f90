program cond_mass_funct_1024_256_halo_num_mpi
  ...
  !
  ! Compile with:
  ! ifx -fpp -DBINARY -mcmodel=medium -shared-intel cond_mass_funct_1024_256_halo_num_mpi.f90 -o cond_mass_funct_1024_256_halo_num_mpi -O3
  !
  ...
  real(kind=dp) :: coarse_dens_coarse(n_region,n_region,n_region), halo_num_coarse(n_region,n_region,n_region)
  character(len=50) :: fname
  real(kind=8) :: halo_num(n_region,n_region,n_region)
       ...
       if (fstat /= 0) then
        ...
        do 
           ...
           if((mass0<=10**lM_min).or.(mass0>10**lM_max))then
            ...
            halo_num(p(1),p(2),p(3))=halo_num(p(1),p(2),p(3)) + 1
           end if
        enddo
...
112     close(15)
     ...
     print*, 'First halo num value: ', halo_num(1,1,1)
      ...
      do dz = 0, block-1
         do dy = 0, block-1
            do dx = 0, block-1
            shift_id = shift_id + 1
            write(fname, '(A,I2.2,A,A,A)') './../binning/coarsened_shift_halo_num_', shift_id, '_', trim(cg_s)//'_z'//trim(z_s), '.dat'
               ...
               do cj = 1, coarsened_grid
                  ...
                  do ii = 0, block-1
                     do jj = 0, block-1
                        ...
                        halo_num_coarse(ci,cj,ck) = halo_num_coarse(ci,cj,ck) + halo_num(i,j,k)
                        end do
                     end do
                  end do
            ...
            print *, ' Saving the coarsened grid: halo_num_coarse(1,1,1)', halo_num_coarse(1,1,1)
            ! Write to file
            open(unit=fh, file=fname, form='formatted')
            do ci = 1, coarsened_grid
               do cj = 1, coarsened_grid
                  do ck = 1, coarsened_grid
                  write(fh, *)  real(coarse_dens_coarse(ci,cj,ck)), real(halo_num_coarse(ci,cj,ck))
                  end do
               end do
            end do
            close(fh)
...
end program cond_mass_funct_1024_256_halo_num_mpi
